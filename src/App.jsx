...
function MainContent() {
  ...
  const handleSend = async () => {
    ...
    try {
        ...
        setChatHistory(prev => [...prev, { type: 'ai', text: "⚠️ Error de conexión con Gemini. Verifica tu API Key o conexión." }]); 
  ...
  if (loading) return <div className="h-screen bg-[#0A0A0A] flex items-center justify-center text-[#7D68F6] font-black tracking-widest animate-pulse uppercase">Cargando Datacenter MRM...</div>;
    ...
    <div className="min-h-screen bg-[#0A0A0A] text-white font-sans selection:bg-[#7D68F6]/30 overflow-x-hidden">
      ...
      <header className="fixed top-0 left-0 w-full p-10 px-12 z-[100] flex justify-between items-start pointer-events-none">
        <div className="flex flex-col items-start cursor-pointer pointer-events-auto" onClick={() => setActiveTab('landing')}>
            ...
            <div className="text-[10px] text-[#7D68F6] mt-1 ml-1 border-l-2 border-[#7D68F6] pl-3 flex flex-col leading-[1.1] uppercase">
        ...
        <div className="flex gap-4 items-center pointer-events-auto">
            {activeTab !== 'landing' && (
                <nav className="flex gap-2 p-2 bg-white/5 backdrop-blur-3xl border border-white/10 rounded-full shadow-2xl mr-4">
                    {[{id:'chat', l:'IA', i:<MessageSquare size={14}/>}, {id:'projects', l:'PROYECTOS', i:<Briefcase size={14}/>}, {id:'team', l:'TALENTO', i:<Users size={14}/>}].map(tab => (
                        <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-6 py-3 rounded-full text-[10px] font-black uppercase transition-all ${activeTab === tab.id ? 'bg-[#7D68F6] text-white' : 'text-white/40'}`}> {tab.i} {tab.l} </button>
                    ))}
                </nav>
            )}
            <div onClick={() => setShowSquadModal(true)} className="bg-[#7D68F6] px-6 py-4 rounded-full cursor-pointer shadow-lg uppercase text-[10px] font-black">SQUAD ({squad.length})</div>
        </div>
      </header>
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'landing' && (
            <motion.section key="landing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex h-screen items-stretch -mt-24">
                {[{ id: 'chat', t: 'CONSULTORÍA IA', i: <MessageSquare size={48}/>, img:'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=1200' }, { id: 'projects', t: 'PROYECTOS', i: <Briefcase size={48}/>, img:'https://images.unsplash.com/photo-1550745165-9bc0b252726f?w=1200' }, { id: 'team', t: 'TALENTO', i: <Users size={48}/>, img:'https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=1200' }].map(card => (
                    <div key={card.id} onClick={() => setActiveTab(card.id)} className="relative flex-1 group cursor-pointer overflow-hidden border-r border-white/5">
                        ...
                        <div className="relative z-10 h-full flex flex-col justify-end p-16 pb-32">
                            <div className="mb-8 text-[#7D68F6]">{card.i}</div>
                            <h2 className="text-5xl font-black uppercase tracking-tighter">{card.t}</h2>
                        </div>
                    </div>
                ))}
            </motion.section>
          ...
          {activeTab === 'chat' && (
            <motion.section key="chat" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto pt-24 w-full px-6 flex flex-col h-[calc(100vh-100px)] pb-12 text-left">
                <div className="relative flex-1 mb-8 overflow-hidden">
                    <div ref={chatContainerRef} className="h-full overflow-y-auto flex flex-col gap-8 hide-scrollbar">
                                    ...
                                    {msg.results && (
                                        <div className="mt-8 pt-8 border-t border-white/10 flex gap-4 overflow-x-auto hide-scrollbar">
                                            {msg.results.map((p, idx) => (
                                                <div key={idx} onClick={() => setSelectedProject(p)} className="min-w-[280px] bg-black/40 border border-white/5 rounded-[2rem] overflow-hidden cursor-pointer hover:border-[#7D68F6]">
                                                    <img src={p.images[0]} className="h-32 w-full object-cover grayscale hover:grayscale-0 transition-all" alt=""/>
                                                    <div className="p-5"><h4 className="text-sm font-black uppercase mb-1">{p.Title}</h4><p className="text-[9px] text-[#7D68F6] font-bold uppercase">VER DETALLES</p></div>
                                                </div>
                                            ))}
                                        </div>
                                    )}
                        ...
                        {isTyping && <div className="text-[10px] font-black text-[#7D68F6] animate-pulse uppercase">Gemini analizando datacenter...</div>}
                    </div>
                </div>
                <div className="flex gap-4">
                    <textarea value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if(e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend(); } }} placeholder="Describe tu necesidad creativa..." className="flex-1 bg-white/5 border border-white/20 rounded-[2.5rem] py-5 px-8 outline-none focus:border-[#7D68F6] transition-all text-[15px] min-h-[64px] backdrop-blur-md resize-none" />
          ...
          {activeTab === 'projects' && (
            <motion.section key="projects" className="pt-48 px-12 max-w-7xl mx-auto pb-40 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                {flatProjects.map((p, i) => (
                    <div key={i} onClick={() => setSelectedProject(p)} className="bg-zinc-900/40 border border-white/5 rounded-[2.5rem] overflow-hidden group cursor-pointer hover:border-[#7D68F6] text-left">
                        ...
                        <div className="p-8"><h4 className="text-xl font-black uppercase text-white mb-2">{p.Title}</h4><p className="text-[10px] text-[#7D68F6] font-black uppercase">VER DETALLES <ChevronRight size={10} className="inline ml-1"/></p></div>
                    </div>
                ))}
            </motion.section>
          )}
                ...
                <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredTalent.map((person, i) => (
                            <div key={i} className="bg-zinc-900/40 border border-white/5 p-8 rounded-[3.5rem] text-center hover:border-[#7D68F6] flex flex-col group">
                                <img src={person.ImageURL} className="w-24 h-24 rounded-full mx-auto mb-6 object-cover grayscale group-hover:grayscale-0 transition-all border-4 border-transparent group-hover:border-[#7D68F6]" alt=""/>
                                <h4 className="text-[18px] font-black uppercase truncate">{person.Name}</h4>
                                <p className="text-[10px] text-[#7D68F6] font-black uppercase mb-4">{person.Role}</p>
                                <button onClick={() => toggleSquad(person)} className={`w-full py-3 rounded-full text-[10px] font-black uppercase border border-[#7D68F6] mt-auto ${squad.some(p => p.ID === person.ID) ? 'bg-[#7D68F6] text-white' : 'text-[#7D68F6] hover:bg-[#7D68F6]/10'}`}>{squad.some(p => p.ID === person.ID) ? 'EN SQUAD' : 'ADD TO SQUAD'}</button>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className="w-full max-w-[1600px] mx-auto my-12 flex flex-col lg:flex-row gap-8 pb-20 text-left">
              <div className="w-full lg:w-[70%] bg-[#0f0f0f] border border-white/10 rounded-[3rem] overflow-hidden shadow-2xl h-fit">
                <div className="relative h-[250px] md:h-[350px] w-full bg-zinc-950 flex overflow-x-auto snap-x hide-scrollbar">
                  {selectedProject.images.map((img, i) => (<img key={i} src={img} className="w-full h-full object-cover flex-shrink-0 snap-start opacity-70" alt=""/>))}
                  <div className="absolute inset-0 bg-gradient-to-t from-[#0f0f0f] to-transparent" />
                </div>
                <div className="p-8 md:p-12 space-y-12">
                  <div className="space-y-6">
                    <span className="text-[10px] font-black uppercase px-4 py-1.5 bg-[#7D68F6]/20 text-[#7D68F6] border border-[#7D68F6]/30 rounded-full">{selectedProject.Category}</span>
                    <h2 className="text-5xl md:text-6xl font-black uppercase tracking-tighter">{selectedProject.Title}</h2>
                    <p className="text-lg text-white/60 normal-case leading-relaxed">{selectedProject.Description}</p>
                    <div className="flex flex-wrap gap-2 pt-2">
                      {selectedProject.tagsArray.map((tag, idx) => (<span key={idx} className="px-4 py-2 bg-zinc-800 text-zinc-300 text-[10px] font-black rounded-full border border-zinc-600 uppercase">{tag}</span>))}
                    </div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-6 border-t border-white/5">
                    {[{t: "LO PEDIDO", d: selectedProject.LoPedido}, {t: "LO HECHO", d: selectedProject.LoHecho}, {t: "LO LOGRADO", d: selectedProject.LoLogrado}].map((col, i) => (
                      <div key={i} className="space-y-4">
                        <h4 className="text-[12px] font-black text-[#7D68F6] uppercase">{col.t}</h4>
                        <p className="text-sm text-white/50 normal-case">{col.d}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="w-full lg:w-[30%] bg-[#0f0f0f] border border-white/10 rounded-[3rem] p-10 shadow-2xl h-fit lg:sticky top-12 flex flex-col">
                <h4 className="text-[12px] font-black uppercase text-[#7D68F6] mb-8">TALENTO INVOLUCRADO</h4>
                <div className="flex flex-col gap-4 mb-10">
                  {activeTeamTalent.length === 0 ? <p className="text-white/30 text-xs normal-case">No se encontró talento asociado. Verifica 'TeamsIDs' en el CSV.</p> : activeTeamTalent.map(member => (
                    <div key={member.ID} className="flex items-center justify-between bg-black/40 p-5 rounded-3xl border border-white/5">
                      <div className="flex items-center gap-4"><img src={member.ImageURL} className="w-12 h-12 rounded-full object-cover" alt=""/><p className="font-black text-[13px] uppercase truncate max-w-[100px]">{member.Name}</p></div>
                      <button onClick={() => toggleSquad(member)} className={`p-3 rounded-full border ${squad.some(s => s.ID === member.ID) ? 'text-red-400 border-red-500/30' : 'text-white/30 border-white/10'}`}>{squad.some(s => s.ID === member.ID) ? <UserMinus size={16}/> : <UserPlus size={16}/>}</button>
                    </div>
                  ))}
                </div>
                <button onClick={toggleEntireTeam} disabled={activeTeamTalent.length === 0} className={`w-full py-5 font-black uppercase text-[10px] rounded-full transition-all ${isEntireTeamInSquad ? 'bg-red-500/10 text-red-400 border border-red-500/30' : 'bg-[#7D68F6] text-white hover:scale-105'}`}>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[300] flex items-center justify-center p-6 backdrop-blur-3xl bg-black/95">
            ...
            <div className="w-full max-w-5xl text-left">
              ...
              <div className="grid grid-cols-12 gap-20">
                <div className="col-span-5 bg-zinc-900/50 p-10 rounded-[3rem] border-l-4 border-[#7D68F6]"><p className="text-white/60 normal-case italic">"Squad optimizado para ejecución estratégica en MRM Bogotá."</p></div>
                <div className="col-span-7">
                  <h4 className="text-[10px] font-black uppercase text-white/40 mb-8 tracking-[0.4em]">PARTICIPANTES ({squad.length})</h4>
                  <div className="flex flex-wrap gap-6 mb-16 overflow-y-auto max-h-[300px] hide-scrollbar">{squad.map(p => (<div key={p.ID} className="text-center"><img src={p.ImageURL} className="w-20 h-20 rounded-full border-2 border-white/5 transition-all mb-3 object-cover" alt=""/><p className="text-[10px] font-black uppercase">{p.Name.split(' ')[0]}</p></div>))}</div>
                  <button onClick={() => { const emails = squad.map(s => s.Email || '').join(';'); window.location.href = `mailto:${emails}?subject=Squad MRM: ${customProjectTitle}`; }} className="w-full py-6 bg-white text-black rounded-full font-black uppercase text-[10px] flex items-center justify-center gap-2 hover:bg-[#7D68F6] hover:text-white transition-all shadow-2xl"><Calendar size={16}/> COORDINAR REUNIÓN TEAMS</button>
                </div>
              </div>
            </div>
          </motion.div>
  ...
  return (
    <div className="h-screen bg-[#0A0A0A] flex flex-col items-center justify-center text-white relative overflow-hidden">
        ...
        <div className="relative z-10 text-center">
            ...
            <button onClick={() => instance.loginRedirect()} className="bg-[#7D68F6] hover:bg-white hover:text-black transition-all duration-300 px-12 py-5 rounded-full text-[12px] font-black uppercase shadow-[0_0_40px_rgba(125,104,246,0.3)]">INICIAR SESIÓN CON MICROSOFT</button>
        </div>
