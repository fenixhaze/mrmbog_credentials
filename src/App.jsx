  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white font-sans selection:bg-[#7D68F6]/30 overflow-x-hidden">
        ...
        <div className="flex gap-4 items-center">
            {activeTab !== 'landing' && (
                <nav className="flex gap-2 p-2 bg-white/5 backdrop-blur-3xl border border-white/10 rounded-full shadow-2xl mr-4">
                    {[ {id: 'chat', label: 'IA Copilot', icon: <MessageSquare size={14}/>}, {id: 'projects', label: 'Proyectos', icon: <Briefcase size={14}/>} ].map(tab => (
            ...
            <motion.div 
                ...
                className="bg-[#7D68F6] px-6 py-4 rounded-full flex items-center gap-4 cursor-pointer shadow-lg shadow-[#7D68F6]/20"
            ...
            >
                ...
                <div className="flex -space-x-3">
                    {squad.slice(0, 4).map((p, idx) => (
                        <img key={idx} src={p.ImageURL} className="w-8 h-8 rounded-full border-2 border-[#7D68F6] bg-black object-cover" alt="sq"/>
                    ))}
                </div>
            </motion.div>
        </div>
                    ...
                    <div ref={chatContainerRef} className="h-full overflow-y-auto pt-10 pb-4 flex flex-col gap-8 hide-scrollbar mask-fade-top scroll-smooth">
                        {chatHistory.map((msg, i) => (
                            <motion.div key={i} className={`flex flex-col ${msg.type === 'user' ? 'items-end' : 'items-start'}`}>
                                <div className={`max-w-[95%] p-6 px-8 rounded-[2rem] text-[15px] border ${msg.type === 'user' ? 'bg-[#7D68F6] border-[#7D68F6]' : 'bg-white/5 border-white/10 backdrop-blur-xl'}`}>
                                    <p className="whitespace-pre-wrap leading-relaxed opacity-90">{msg.text}</p>
                                    
                                    {/* RESPUESTA INTEGRADA: PROYECTOS SUGERIDOS */}
                                            ...
                                            <div className="flex gap-4 overflow-x-auto pb-4 hide-scrollbar snap-x">
                                                {msg.results.map((project, idx) => (
                                                    <div key={idx} onClick={() => setSelectedProject(project)} className="min-w-[280px] bg-black/40 border border-white/5 rounded-[2rem] overflow-hidden group cursor-pointer hover:border-[#7D68F6] transition-all shadow-2xl">
                                                        ...
                                                        <div className="p-5">
                                                            <h4 className="text-sm font-black uppercase mb-1 leading-tight">{project.Title}</h4>
                                    ...
                                    {/* RESPUESTA INTEGRADA: SQUAD RECOMENDADO */}
                                        ...
                                        <div className="mt-6">
                                            ...
                                            <div className="flex gap-4 overflow-x-auto pb-2 hide-scrollbar">
                                                {msg.recommendedTalent.map((t, idx) => (
                                                    <div key={idx} className="min-w-[160px] bg-black/40 p-5 rounded-[2rem] border border-white/5 text-center group relative">
                                                        <div className="w-16 h-16 rounded-full mx-auto mb-3 overflow-hidden border border-white/10 group-hover:border-[#7D68F6] transition-all">
                                                        ...
                                                        <p className="text-[8px] text-[#7D68F6] font-black uppercase mb-4">{t.Role}</p>
                        ...
                        {isTyping && <div className="text-white/20 font-black italic uppercase text-xs tracking-widest animate-pulse ml-4">El Copilot está pensando...</div>}
                    </div>
                </div>
          ...
          {activeTab === 'projects' && (
            <motion.section key="projects" className="pt-48 px-12 max-w-7xl mx-auto pb-40 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                {flatProjects.map((p, i) => (
                    <motion.div key={i} whileHover={{ y: -5 }} onClick={() => setSelectedProject(p)} className="bg-zinc-900/40 border border-white/5 rounded-[2.5rem] overflow-hidden group cursor-pointer hover:border-[#7D68F6] transition-all">
                        ...
                        <div className="p-8">
                            <h4 className="text-xl font-black uppercase text-white mb-2">{p.Title}</h4>
                            <p className="text-[10px] text-[#7D68F6] font-bold uppercase tracking-widest">Ver Detalles <ChevronRight size={10} className="inline ml-1"/></p>
                        </div>
                    </motion.div>
                ))}
            </motion.section>
            ...
            <div className="bg-[#0f0f0f] border border-white/10 w-full max-w-6xl h-[90vh] rounded-[3rem] overflow-hidden relative flex flex-col shadow-2xl">
              ...
              <div className="p-16 flex-1 overflow-y-auto grid grid-cols-12 gap-16 hide-scrollbar">
                <div className="col-span-7">
                  <div className="flex flex-wrap gap-2 mb-6">
                    {selectedProject.tagsArray?.map(tag => (
                      <span key={tag} className="text-[9px] font-black uppercase px-4 py-1.5 bg-[#7D68F6]/10 text-[#7D68F6] border border-[#7D68F6]/20 rounded-full">{tag}</span>
                    ))}
                  </div>
                  <h2 className="text-6xl font-black italic uppercase mb-8 tracking-tighter">{selectedProject.Title}</h2>
                ...
                <div className="col-span-5 bg-white/5 p-10 rounded-[2.5rem] border border-white/5 flex flex-col">
                  ...
                  <div className="space-y-4 mb-10 overflow-y-auto max-h-[300px] pr-2 hide-scrollbar">
                      ...
                      <div key={member.ID} className="flex items-center justify-between group">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-white/40 font-bold uppercase">{member.Role}</p>
                          </div>
                        </div>
                  ...
                  <button onClick={() => addEntireTeamToSquad(selectedProject.teamArray || [])} className="mt-auto py-5 bg-[#7D68F6] text-white font-black uppercase tracking-[0.2em] text-[11px] rounded-full hover:scale-105 transition-all flex items-center justify-center gap-3">
        ...
        {showSquadModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[300] flex items-center justify-center p-6 backdrop-blur-3xl bg-black/95">
            ...
            <div className="w-full max-w-5xl">
              <input value={customProjectTitle} onChange={(e) => setCustomProjectTitle(e.target.value)} className="bg-transparent text-7xl font-black italic uppercase border-b-2 border-white/10 focus:border-[#7D68F6] outline-none w-full pb-6 mb-16 tracking-tighter" placeholder="Escribe el nombre del proyecto..."/>
              <div className="grid grid-cols-12 gap-20">
                <div className="col-span-5 bg-zinc-900/50 p-10 rounded-[3rem] border-l-4 border-[#7D68F6]">
                  ...
                  <p className="text-white/60 italic text-xl leading-relaxed">
                    "Análisis completado. El equipo seleccionado garantiza una cobertura técnica total de los requerimientos de MRM Bogotá."
                  </p>
                </div>
                <div className="col-span-7">
      ...
      <style>{`
        ...
        h1, h2, h3, h4, .font-black { font-weight: 900 !important; }
