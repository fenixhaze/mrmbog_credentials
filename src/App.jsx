  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setTalentData(Papa.parse(talentCSV, { header: true, skipEmptyLines: true, delimiter: ";" }).data);
        ...
        setLoading(false);
        setChatHistory([{ type: 'ai', text: `Consultoría Creativa MRM activa. ¿Qué perfiles y proyectos necesitas para tu próximo pitch?` }]);
    ...
    try {
        const response = await fetch(POWER_AUTOMATE_URL, {
            ...
            body: JSON.stringify({ 
                ...
                Talento: JSON.stringify(talentData.slice(0, 30)) // Enviamos talento también
            })
        });
        ...
        try {
            ...
            if (jsonOnly) {
                ...
                finalTalentIds = parsed.talent_ids || [];
        ...
        const matchedTalent = talentData.filter(t => finalTalentIds.includes(t.Name) || finalTalentIds.includes(t.ID));

        setChatHistory(prev => [...prev, { 
            type: 'ai', 
            text: cleanReason, 
            results: matchedProjects,
            talent: matchedTalent // Guardamos talento en el historial
        }]);

    } catch { 
        setChatHistory(prev => [...prev, { type: 'ai', text: "Hubo un problema analizando el equipo ideal. Inténtalo de nuevo." }]); 
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'landing' && (
            <motion.section key="landing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 flex items-stretch h-screen overflow-hidden">
              {[
                ...
                { id: 'projects', title: 'Proyectos', desc: 'Explora nuestras mejores piezas.', icon: <Briefcase size={48}/>, img: 'https://images.unsplash.com/photo-1550745165-9bc0b252726f?auto=format&fit=crop&q=80&w=2070' },
                { id: 'team', title: 'Talento', desc: 'Las mentes creativas detrás de MRM.', icon: <Users size={48}/>, img: 'https://images.unsplash.com/photo-1522071820081-009f0129c71c?auto=format&fit=crop&q=80&w=2070' }
              ].map((card) => (
                        ...
                        {chatHistory.map((msg, i) => (
                            <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className={`flex flex-col ${msg.type === 'user' ? 'items-end' : 'items-start'}`}>
                                <div className={`max-w-[85%] p-8 px-10 rounded-[3rem] text-[15px] border ${msg.type === 'user' ? 'bg-[#7D68F6] border-[#7D68F6] rounded-tr-none shadow-[#7D68F6]/20' : 'bg-white/5 border-white/10 backdrop-blur-xl rounded-tl-none shadow-2xl'}`}>
                                    ...
                                    {msg.results && msg.results.length > 0 && (
                                        <div className="mt-8 flex gap-5 overflow-x-auto pb-6 hide-scrollbar snap-x snap-mandatory border-t border-white/5 pt-8">
                                            {msg.results.map((project, idx) => (
                                                <div key={idx} onClick={() => setSelectedProject(project)} className="min-w-[260px] w-[260px] flex-shrink-0 snap-start bg-black/40 border border-white/10 rounded-3xl overflow-hidden group cursor-pointer hover:border-[#7D68F6] transition-all">
                                                    ...
                                                    <div className="p-5"><h4 className="text-[11px] font-black uppercase tracking-tighter mb-1 truncate">{project.ProjectName}</h4><p className="text-[8px] text-[#7D68F6] font-bold uppercase tracking-widest truncate">{project.Client}</p></div>
                                                </div>
                                            ))}
                                        </div>
                                    )}
                                    ...
                                    {/* TALENTO RECOMENDADO */}
                                    {msg.talent && msg.talent.length > 0 && (
                                        <div className="mt-6 pt-6 border-t border-white/5">
                                            <h5 className="text-[9px] font-black uppercase tracking-[0.4em] text-[#7D68F6] mb-6 flex items-center gap-2"><Star size={12}/> El Equipo Ideal</h5>
                                            <div className="flex gap-4 overflow-x-auto hide-scrollbar pb-2">
                                                {msg.talent.map((t, idx) => (
                                                    <div key={idx} className="flex flex-col items-center min-w-[100px] text-center group">
                                                        <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-transparent group-hover:border-[#7D68F6] transition-all mb-3 shadow-xl">
                                                            <img src={t.ImageURL} className="w-full h-full object-cover grayscale group-hover:grayscale-0 group-hover:scale-110 transition-all duration-700" alt="avatar"/>
                                                        </div>
          ...
          {activeTab === 'team' && (
            <motion.section key="team" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-16 items-start pt-48 px-12 max-w-7xl mx-auto pb-40">
                <aside className="w-64 sticky top-48 space-y-10"><div><h3 className="text-[#7D68F6] text-[10px] font-black uppercase tracking-[0.3em] mb-6 flex items-center gap-2"><Filter size={14}/> Filtrar Rol</h3><div className="flex flex-col gap-2">{uniqueRoles.map(role => (<button key={role} onClick={() => setFilterRole(role)} className={`text-left px-5 py-2.5 rounded-full text-[11px] font-bold uppercase transition-all ${filterRole === role ? 'bg-[#7D68F6] text-white shadow-md' : 'text-white/30 hover:text-white hover:bg-white/5'}`}>{role}</button>))}</div></div></aside>
                <div className="flex-1"><div className="mb-12"><h2 className="text-7xl font-black italic uppercase tracking-tighter leading-none">Equipo Bogotá</h2></div><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">{filteredTalent.map((person, i) => (
                    <motion.div key={i} whileHover={{ y: -5 }} className="bg-white/5 border border-white/10 p-8 rounded-[3.5rem] text-center hover:border-[#7D68F6] transition-all group overflow-hidden"><div className="w-24 h-24 rounded-full mx-auto mb-6 overflow-hidden border-4 border-transparent group-hover:border-[#7D68F6] transition-all duration-500 shadow-xl"><img src={person.ImageURL} className="w-full h-full object-cover grayscale group-hover:grayscale-0 group-hover:scale-110 transition-all duration-700" alt="avatar"/></div><h4 className="text-xl font-black uppercase mb-1 tracking-tighter leading-none">{person.Name}</h4><p className="text-[10px] text-[#7D68F6] font-bold uppercase mb-6 tracking-widest">{person.Role}</p></motion.div>))}</div></div>
            </motion.section>
          )}
        </AnimatePresence>
      </main>
