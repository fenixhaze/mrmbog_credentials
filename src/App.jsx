...
function MainContent() {
    ...
    const fetchData = async () => {
      try {
        ...
        setFlatProjects(rawProjects.map(p => ({
            ...
            teamArray: String(p.TeamsIDs || p.TeamIDs || "").replace(/;/g, ',').split(',').map(t => t.trim()).filter(Boolean),
            Category: p.Category || "Proyecto Especial",
            LoPedido: p.LoPedido || "Ejecución técnica e integración visual según requerimientos.",
            LoHecho: p.LoHecho || "Desarrollo de ecosistema digital de alta performance.",
            LoLogrado: p.LoLogrado || "Plataforma desplegada con excelente interactividad de usuario."
        })));

        setChatHistory([{ type: 'ai', text: `Sistema MRM Bogotá en línea. Gemini AI conectado. ¿En qué puedo ayudarte hoy?` }]);
        setLoading(false);
      } catch (e) { console.error("Error carga:", e); setLoading(false); }
    };
    fetchData();
  }, [instance, accounts]);
  ...
  const handleSend = async () => {
    ...
    try {
        ...
        const invLite = JSON.stringify(flatProjects.slice(0, 50).map(p => ({ id: p.ID, t: p.Title, c: p.tagsArray.join(',') })));
        const talLite = JSON.stringify(talentData.slice(0, 50).map(t => ({ n: t.Name, r: t.Role, h: t.skillsArray.join(',') })));

        const promptText = `Eres un experto en staffing para MRM Bogotá. Necesidad: "${userMsg}". Proyectos: ${invLite}. Talento: ${talLite}. Devuelve SOLO un JSON (sin markdown): {"match_ids": ["ID1"], "talent_names": ["Nombre1"], "reason": "Breve explicación."}`;
        ...
        setChatHistory(prev => [...prev, { type: 'ai', text: "⚠️ Error de conexión con Gemini. Intenta de nuevo." }]); 
  ...
  if (loading) return <div className="h-screen bg-[#0A0A0A] flex items-center justify-center text-[#7D68F6] font-black uppercase tracking-widest animate-pulse">CARGANDO...</div>;
    ...
    <div className="min-h-screen bg-[#0A0A0A] text-white font-sans selection:bg-[#7D68F6]/30 overflow-x-hidden">
      ...
      <header className="fixed top-0 left-0 w-full p-10 px-12 z-[100] flex justify-between items-start pointer-events-none">
        <div className="flex flex-col items-start cursor-pointer pointer-events-auto" onClick={() => setActiveTab('landing')}>
            ...
            <div className="text-[10px] text-[#7D68F6] mt-1 ml-1 border-l-2 border-[#7D68F6] pl-3 flex flex-col leading-[1.1] tracking-[-0.02em] font-normal uppercase">
        ...
        <div className="flex gap-4 items-center pointer-events-auto">
            {activeTab !== 'landing' && (
                <nav className="flex gap-2 p-2 bg-white/5 backdrop-blur-3xl border border-white/10 rounded-full shadow-2xl mr-4">
                    {[{id: 'chat', label: 'CONSULTORÍA IA', icon: <MessageSquare size={14}/>}, {id: 'projects', label: 'PROYECTOS', icon: <Briefcase size={14}/>}, {id: 'team', label: 'TALENTO', icon: <Users size={14}/>}].map(tab => (
            ...
            <motion.div onClick={() => setShowSquadModal(true)} className="bg-[#7D68F6] px-6 py-4 rounded-full flex items-center gap-4 cursor-pointer shadow-lg" whileHover={{ scale: 1.05 }}>
                <span className="text-[10px] font-black uppercase tracking-widest">SQUAD ({squad.length})</span>
            </motion.div>
        </div>
      </header>
      ...
      <main className="relative z-10 min-h-screen pt-24">
        <AnimatePresence mode="wait">
          {activeTab === 'landing' && (
            <motion.section key="landing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex h-screen items-stretch -mt-24">
          ...
          {activeTab === 'chat' && (
            <motion.section key="chat" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto pt-24 w-full px-6 flex flex-col h-[calc(100vh-100px)] pb-12 text-left">
                <div className="relative flex-1 mb-8 overflow-hidden">
                    <div ref={chatContainerRef} className="h-full overflow-y-auto pt-10 pb-4 flex flex-col gap-8 hide-scrollbar scroll-smooth">
                                            ...
                                            <div className="flex gap-4 overflow-x-auto pb-4 hide-scrollbar">
                                                {msg.results.map((project, idx) => (
                                                    <div key={idx} onClick={() => setSelectedProject(project)} className="min-w-[280px] bg-black/40 border border-white/5 rounded-[2rem] overflow-hidden group cursor-pointer hover:border-[#7D68F6] transition-all">
                                                        ...
                                                        <div className="p-5"><h4 className="text-sm font-black uppercase mb-1">{project.Title}</h4><p className="text-[9px] text-[#7D68F6] font-bold uppercase tracking-widest">VER DETALLES</p></div>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                        ...
                        {isTyping && <div className="text-[10px] font-bold uppercase text-[#7D68F6] animate-pulse">GEMINI PENSANDO...</div>}
                    </div>
                </div>
                <div className="flex items-center gap-4 w-full">
                    ...
                    <button onClick={handleSend} className="bg-[#7D68F6] w-[64px] h-[64px] rounded-full flex items-center justify-center flex-shrink-0 hover:scale-105 transition-all"><Send size={22}/></button>
                </div>
            </motion.section>
          )}
          ...
          {activeTab === 'projects' && (
            <motion.section key="projects" className="pt-48 px-12 max-w-7xl mx-auto pb-40 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                {flatProjects.map((p, i) => (
                    <motion.div key={i} whileHover={{ y: -5 }} onClick={() => setSelectedProject(p)} className="bg-zinc-900/40 border border-white/5 rounded-[2.5rem] overflow-hidden group cursor-pointer hover:border-[#7D68F6] transition-all shadow-xl text-left">
          ...
          {activeTab === 'team' && (
            <motion.section key="team" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-16 items-start pt-48 px-12 max-w-7xl mx-auto pb-40 text-left">
                <aside className="w-64 sticky top-48 space-y-10">
                    <h3 className="text-[#7D68F6] text-[10px] font-black uppercase tracking-[0.4em]">FILTRAR ROL</h3>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredTalent.map((person, i) => (
                            <motion.div key={i} whileHover={{ y: -5 }} className="bg-zinc-900/40 border border-white/5 p-8 rounded-[3.5rem] text-center hover:border-[#7D68F6] transition-all group overflow-hidden flex flex-col shadow-lg">
                                <div className="w-24 h-24 rounded-full mx-auto mb-6 overflow-hidden border-4 border-transparent group-hover:border-[#7D68F6] transition-all bg-black"><img src={person.ImageURL} className="w-full h-full object-cover grayscale group-hover:grayscale-0" alt=""/></div>
                                ...
                                <button onClick={() => toggleSquad(person)} className={`w-full py-3 rounded-full text-[10px] font-black uppercase border border-[#7D68F6] transition-all mt-auto ${squad.some(p => p.ID === person.ID) ? 'bg-[#7D68F6] text-white shadow-lg' : 'text-[#7D68F6] hover:bg-[#7D68F6]/10'}`}>{squad.some(p => p.ID === person.ID) ? 'EN SQUAD' : 'ADD TO SQUAD'}</button>
                            </motion.div>
                        ))}
                    </div>
                </div>
      ...
      <AnimatePresence>
        {selectedProject && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex items-start justify-center p-6 backdrop-blur-2xl bg-black/80 pointer-events-auto overflow-y-auto">
            ...
            <div className="w-full max-w-[1600px] mx-auto my-12 flex flex-col lg:flex-row gap-8 pb-20 relative text-left">
              <div className="w-full lg:w-[70%] bg-[#0f0f0f] border border-white/10 rounded-[3rem] overflow-hidden shadow-2xl h-fit">
                <div className="relative h-[250px] md:h-[350px] w-full bg-zinc-950 flex snap-x hide-scrollbar overflow-x-auto">
                  {selectedProject.images.map((img, i) => (<img key={i} src={img} className="w-full h-full object-cover flex-shrink-0 snap-start opacity-70" alt="S" />))}
                ...
                <div className="p-8 md:p-12 space-y-12">
                  <div className="space-y-6">
                    ...
                    <h2 className="text-5xl md:text-6xl font-black uppercase tracking-tighter text-white">{selectedProject.Title}</h2>
                    <p className="text-lg text-white/60 font-normal normal-case leading-relaxed">{selectedProject.Description}</p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-6 border-t border-white/5">
                    {[{t: "LO PEDIDO", d: selectedProject.LoPedido}, {t: "LO HECHO", d: selectedProject.LoHecho}, {t: "LO LOGRADO", d: selectedProject.LoLogrado}].map((col, i) => (
                      <div key={i} className="space-y-4">
                        ...
                        <p className="text-sm text-white/50 font-normal normal-case">{col.d}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="w-full lg:w-[30%] bg-[#0f0f0f] border border-white/10 rounded-[3rem] p-10 shadow-2xl h-fit lg:sticky top-12 flex flex-col">
                ...
                <div className="flex flex-col gap-4 mb-10">
                  {activeTeamTalent.length === 0 ? <p className="text-white/30 text-xs">No hay talentos asignados.</p> : activeTeamTalent.map(member => (
                    <div key={member.ID} className="flex items-center justify-between bg-black/40 p-5 rounded-3xl border border-white/5">
                      <div className="flex items-center gap-4"><img src={member.ImageURL} className="w-12 h-12 rounded-full object-cover" alt=""/><p className="font-black text-[13px] uppercase text-white truncate max-w-[100px]">{member.Name}</p></div>
            ...
            <div className="w-full max-w-5xl text-left">
              ...
              <div className="grid grid-cols-12 gap-20">
                ...
                <div className="col-span-7">
                  ...
                  <div className="flex flex-wrap gap-6 mb-16">{squad.map(p => (<div key={p.ID} className="text-center group"><img src={p.ImageURL} className="w-20 h-20 rounded-full border-2 border-white/5 group-hover:border-[#7D68F6] transition-all mb-3 object-cover" alt=""/><p className="text-[10px] font-black uppercase">{p.Name.split(' ')[0]}</p></div>))}</div>
