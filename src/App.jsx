...
function MainContent() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setChatHistory([{ type: 'ai', text: `Sistema MRM Bogotá activo. Datacenter cargado y listo para staffing estratégico.` }]);
        setLoading(false);
  ...
  const handleSend = async () => {
    ...
    try {
        ...
        const tBrief = talentData.slice(0, 20).map(t => `${t.Name} (${t.Role})`).join(", ");

        const prompt = `Eres experto en MRM Bogotá. Datos: Proyectos(${pBrief}), Talento(${tBrief}). 
        ...
        setChatHistory(prev => [...prev, { type: 'ai', text: `⚠️ Error de conexión con IA.` }]); 
  ...
  if (loading) return <div className="h-screen bg-[#0A0A0A] flex items-center justify-center text-[#7D68F6] font-black uppercase animate-pulse text-2xl">MRM CREDENTIALS...</div>;
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white font-sans overflow-x-hidden selection:bg-[#7D68F6]/30">
                                        ...
                                        <div className="flex flex-wrap gap-3 mt-2 pt-5 border-t border-white/10">
                                            {msg.recommendedTalent.map((t, idx) => (
                                                <div key={idx} className="bg-black/40 p-3 pr-5 rounded-full border border-white/5 flex items-center gap-4 group">
                                                    ...
                                                    <button onClick={() => toggleSquad(t)} className={`ml-2 p-2 rounded-full border transition-all ${squad.some(s => s.ID === t.ID) ? 'bg-[#7D68F6] border-[#7D68F6] text-white' : 'border-white/10 text-white/40 hover:text-white hover:border-white'}`}>
          ...
          {activeTab === 'team' && (
            <motion.section key="team" className="flex gap-16 pt-48 px-12 max-w-7xl mx-auto pb-40 text-left">
                <aside className="w-64 sticky top-48 flex flex-col gap-2">
                    <h3 className="text-[#7D68F6] text-[10px] font-black uppercase mb-8 tracking-widest tracking-widest">ROLES</h3>
      ...
      {/* MODAL DETALLE PROYECTO (RECUPERADO 70/30) */}
      <AnimatePresence>
        {selectedProject && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex items-start justify-center p-6 backdrop-blur-2xl bg-black/90 overflow-y-auto">
            ...
            <div className="w-full max-w-[1600px] mx-auto my-12 flex flex-col lg:flex-row gap-8 pb-20 text-left">
              ...
              {/* COLUMNA 70%: INFO DEL PROYECTO */}
              <div className="w-full lg:w-[70%] bg-[#0f0f0f] border border-white/10 rounded-[3rem] overflow-hidden shadow-2xl">
                <div className="relative h-[400px] w-full bg-zinc-950 flex overflow-x-auto snap-x hide-scrollbar">
              ...
              {/* COLUMNA 30%: TALENTO (BARRA LATERAL RECUPERADA) */}
              <div className="w-full lg:w-[30%] bg-[#0f0f0f] border border-white/10 rounded-[3rem] p-10 shadow-2xl h-fit lg:sticky top-12 flex flex-col">
                <h4 className="text-[12px] font-black uppercase tracking-[0.4em] text-[#7D68F6] mb-8">EQUIPO INVOLUCRADO</h4>
                <div className="flex flex-col gap-4 mb-10 max-h-[50vh] overflow-y-auto hide-scrollbar">
                  {activeTeamTalent.length === 0 ? <p className="text-white/30 text-xs italic">No hay talentos vinculados.</p> : activeTeamTalent.map(member => (
                    <div key={member.ID} className="flex items-center justify-between bg-black/40 p-5 rounded-3xl border border-white/5 transition-all">
                      ...
                      <button onClick={() => toggleSquad(member)} className={`p-3 rounded-full border transition-all ${squad.some(s => s.ID === member.ID) ? 'text-red-400 border-red-500/30 bg-red-500/10' : 'text-white/30 border-white/10 hover:text-[#7D68F6] hover:border-[#7D68F6]'}`}>
                ...
                <button onClick={toggleEntireTeam} disabled={activeTeamTalent.length === 0} className={`w-full py-5 font-black uppercase text-[10px] rounded-full transition-all ${isEntireTeamInSquad ? 'bg-red-500/10 text-red-400 border border-red-500/30' : 'bg-[#7D68F6] text-white hover:scale-105 shadow-lg shadow-[#7D68F6]/20'}`}>
                    {isEntireTeamInSquad ? 'RETIRAR EQUIPO COMPLETO' : 'AGREGAR EQUIPO COMPLETO'}
                </button>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[300] flex items-center justify-center p-6 backdrop-blur-3xl bg-black/95">
            ...
            <div className="w-full max-w-5xl text-left">
              ...
              <div className="grid grid-cols-12 gap-20">
                <div className="col-span-5 bg-zinc-900/50 p-10 rounded-[3rem] border-l-4 border-[#7D68F6] shadow-xl"><p className="text-[#7D68F6] font-black uppercase tracking-widest text-[10px] mb-4">ANÁLISIS DE SQUAD</p><p className="text-white/60 italic leading-relaxed">"Configuración estratégica basada en credenciales MRM."</p></div>
                <div className="col-span-7">
                  <h4 className="text-[10px] font-black uppercase text-white/40 mb-8 tracking-[0.4em]">TALENTOS SELECCIONADOS ({squad.length})</h4>
...
function LoginScreen() {
  ...
  return (
    <div className="h-screen bg-[#0A0A0A] flex flex-col items-center justify-center text-white relative overflow-hidden">
        ...
        <style>{`body, html { font-family: 'Inter', sans-serif !important; background-color: #0A0A0A !important; } h1, h2, h3, h4, .font-black { font-weight: 900 !important; }`}</style>
    </div>
  );
}
