...
function MainContent() {
  ...
  const handleSend = async () => {
    ...
    try {
        ...
        // Contextos externos separados
        const pContext = flatProjects.slice(0, 15).map(p => `ID:${p.ID} | Proyecto:${p.Title} | Tags:${p.tags}`).join("\n");
        const tContext = talentData.slice(0, 15).map(t => `Nombre:${t.Name} | Rol:${t.Role} | Skills:${t.Tags}`).join("\n");

        const prompt = `Staffing Manager MRM Bogotá.
        [CREDENTIALS DATA]
        ${pContext}
        [TALENT DATA]
        ${tContext}
        ...
        USER: "${userMsg}"
        TASK: Recommend projects and MAX 4 people.
        FORMAT: {"match_ids":[], "talent_names":[], "reason":""}`;
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white font-sans overflow-x-hidden selection:bg-[#7D68F6]/30">
      ...
      {/* HEADER + BANNER DE NAVEGACIÓN RECUPERADO */}
      <header className="fixed top-0 left-0 w-full p-10 px-12 z-[100] flex justify-between items-start pointer-events-none">
        ...
        {/* Banner de Navegación (Solo visible fuera de landing) */}
      ...
      <main className="relative z-10 min-h-screen pt-24 pb-20">
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'chat' && (
            <motion.section key="chat" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto pt-24 w-full px-6 flex flex-col h-[calc(100vh-100px)] text-left">
                                ...
                                {msg.recommendedTalent && msg.recommendedTalent.length > 0 && (
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2 pt-5 border-t border-white/10">
                                        {msg.recommendedTalent.map((t, idx) => (
                                            <div key={idx} className="bg-black/40 p-3 pr-5 rounded-full border border-white/5 flex items-center justify-between group transition-all hover:ring-2 hover:ring-[#7D68F6]">
                ...
                <div className="flex gap-4">
                    <textarea value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if(e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend(); } }} placeholder="Dime qué buscas..." className="flex-1 bg-white/5 border border-white/20 rounded-[2.5rem] py-5 px-8 outline-none focus:border-[#7D68F6] text-[15px] min-h-[64px] backdrop-blur-md resize-none" />
                ...
                <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredTalent.map((person, i) => (
                            <div key={i} className="bg-zinc-900/40 border border-white/5 p-8 rounded-[3.5rem] text-center flex flex-col group transition-all hover:ring-2 hover:ring-[#7D68F6]">
        ...
        {selectedProject && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex items-start justify-center p-6 backdrop-blur-2xl bg-black/95 overflow-y-auto">
            ...
            <div className="w-full max-w-[1600px] mx-auto my-12 flex flex-col lg:flex-row gap-8 pb-20 text-left relative">
              ...
              {/* BARRA LATERAL TALENTO CON HOVER STROKE */}
              <div className="w-full lg:w-[30%] bg-[#0f0f0f] border border-white/10 rounded-[3rem] p-10 shadow-2xl h-fit lg:sticky top-12 flex flex-col">
                ...
                <div className="flex flex-col gap-4 max-h-[50vh] overflow-y-auto hide-scrollbar">
                  {activeTeamTalent.map(member => (
                    <div key={member.ID} className="flex items-center justify-between bg-black/40 p-5 rounded-3xl border border-white/5 group transition-all hover:ring-2 hover:ring-[#7D68F6]">
                      <div className="flex items-center gap-4 text-left">
                      ...
                      <button onClick={() => toggleSquad(member)} className={`p-3 rounded-full border transition-all ${squad.some(s => s.ID === member.ID) ? 'bg-[#7D68F6] text-white shadow-lg' : 'text-white/30 border-white/10 hover:text-white'}`}>
